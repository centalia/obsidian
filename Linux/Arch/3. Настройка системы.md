...
### 3.3.8 Автоматическая очистка кэша pacman
Кэш пакетов pacman имеет очень хуёвое свойство забиваться и со временем занимает много места на диске. Решается самописным демоном:
...
### 3.3.10 Отрубаем к хуям мусорные службы
...
# 3.3 Установка и настройка Pipewire
...
~~~
sudo pacman -S pipewire pipewire-pulse pipewire-jack/jack2 lib32-jack2/lib32-pipewire-jack lib32-pipewire gst-plugin-pipewire wireplumber realtime-privileges rtkit && systemctl --user enable --now pipewire pipewire.socket pipewire-pulse wireplumber
~~~

- Добавляем себя в группу `realtime`
~~~
sudo usermod -aG realtime "$USER"
groups %USER% // Для проверки
~~~
...
```
mkdir -p ~/.config/pipewire/pipewire-pulse.conf.d && cp /usr/share/pipewire/client-rt.conf.avail/20-upmix.conf ~/.config/pipewire/pipewire-pulse.conf.d && cp /usr/share/pipewire/client-rt.conf.avail/20-upmix.conf ~/.config/pipewire/client-rt.conf.d
```

- Решаем хрипы под нагрузкой

Для их исправления отредактируем файл `10-sound.conf`:

Здесь меняем только значение параметра `quantum` до 4096. Остальные значения как `default.clock.rate` и `default.clock.allowed-rates` меняем с учетом вашего оборудования.

- Jack
Существует три различных реализации JACK API: просто jack из AUR, jack2 и pipewire-jack.

Для альтернативной поддержки ALSA MIDI в jack2 установите [a2jmidid.](https://archlinux.org/packages/community/x86_64/a2jmidid/).
Для поддержки dbus с jack2 установите [jack2-dbus](https://archlinux.org/packages/extra/x86_64/jack2-dbus/) 
