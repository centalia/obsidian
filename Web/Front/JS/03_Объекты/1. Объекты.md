...
Дабы избежать такого есть методы поверхностного копирования и методы глубокого копирования 

Метод поверхностного копирования через `Object.assign()`
...
Метод поверхностного копирования через `...` (оператор разделения объекта)
...
Но есть одно НО! Оба метода не защищают от мутации вложенных объектов. Если у объекта есть вложенные объекты, то ссылки на них сохраняются. 

Метод глубокого копирования через JSON.parse(JSON.stringify())
...
user['Xyi Sosat'] = false;
user.countXyi = 0;

console.log(user); //{ name: 'Алексей', age: 21, 'Xyi Sosat': false, countXyi: 0 }
console.log(userNew); /* {
  name: 'Алексей',
  age: 21,
  'Xyi Sosat': true,
  countXyi: { count: 9999, type: 'big' }
} */
```